...
public class Path implements Serializable{
	 private static final File dir = new File("Export");
	    ...
	    public void addPI( InterestPoint p ){m_path.add(p);}
	    ...
	    public String calculerItinerairePiObligatoire(InterestPoint depart, InterestPoint arrive, InterestPoint piObligatoire, float budget){
	        ...
	        if(p<budget){
	            return ("Itinéraire calculé : \nVous partirez de " + depart.getName()
	                    ...
	                    + "\nVous arriverez à " + arrive.getName()
	                    + "\nCout estimé : " + p + "€ (dont " + (55*costTransport) + "€ cout de transport)");
	        }
	        return ("Budget insuffisant. \nCout estimé : " + p);
	    }

	    public String calculerItinerairePi(InterestPoint depart, InterestPoint arrive, InterestPoint piObligatoire, InterestPoint piInterdit, float budget){
	    ...
	    public void m_pathWriter(String nomFichier) throws IOException {
	        ...
	        pw.close();
	        System.out.println("\nLe fichier a bien été créé. \nVoici son emplacement : " + System.getProperty("user.dir") +"\\" + dir + "\\" + nomFichier + ".txt");
	    }
	    ...
	    public double distance(InterestPoint p1, InterestPoint p2) {
	        ...
	        double dist = Math.sin(deg2rad(p1.getLat())) * Math.sin(deg2rad(p2.getLat())) + Math.cos(deg2rad(p1.getLon())) * Math.cos(deg2rad(p2.getLon())) * Math.cos(deg2rad(theta));
	        dist = Math.acos(dist);
	        dist = rad2deg(dist);
	        dist = dist * 60 * 1.1515;
	        return (dist);
